...
library TokenHelper {
    function callTokenFunction(address tokenAddress, bytes memory data,bool staticCall) private  returns (bytes memory) {
    ...
    function approveToken(address token,address spender,uint spendAmount) internal returns (bool){
       bytes memory result =  callTokenFunction(token,abi.encodeWithSignature("approve(address,uint256)",spender,spendAmount),false);
       return abi.decode(result, (bool));
    }

    function transferToken(address token,address receiver,uint amount) internal returns (bool) {
        bytes memory result =  callTokenFunction(token,abi.encodeWithSignature("transfer(address,uint256)",receiver,amount),false);
        return abi.decode(result, (bool));
    }
}